...
agmt = Arrangement(body_type = "fuselage",           # "fuselage"
                   ...
                   tank_architecture = "rear",   # "wing_box", "under_floor", "rear", "piggy_back" or "pods"
                   gear_architecture = "retractable",# "retractable", "bare_fixed"
                   number_of_engine = "twin",        # "twin" or "quadri"
                   nacelle_attachment = "rear",      # "wing", "pod" "rear" or "body_cones"
                   power_architecture = "tf",        # "tf", "tp", "ef", "ep", "pte", "pte_pod", "pte_piggy"
                   power_source = "fuel",            # "fuel", "fuel_cell", "fuel_cell_PEM", "battery"
                   fuel_type = "liquid_h2")           # "kerosene", "liquid_h2", "compressed_h2" or "battery"

cruise_altp = unit.m_ft(35000.)
cruise_mach = 0.78    #max0.82
reqs = Requirement(n_pax_ref = 96.,
                   design_range = unit.m_NM(570.),
...
#-----------------------------------------------------------------------------------------------------------------------
# Take off
ac.requirement.take_off.tofl_req = 2090.                                   #The takeoff field length consists of two parts: takeoff ground roll and takeoff path to 35 ft height

# Approach
ac.requirement.approach.app_speed_req = unit.convert_from("kt",135.)
# Climb
ac.requirement.mcl_ceiling.altp = cruise_altp
ac.requirement.mcl_ceiling.mach = cruise_mach * 0.8
ac.requirement.mcl_ceiling.vz_req = unit.convert_from("ft/min",3000.)
...
ac.power_system.reference_thrust = unit.N_kN(115.)

ac.airframe.wing.hld_type = 9.
...
var_bnd = [[unit.N_kN(100.), unit.N_kN(500.)],       # Design space area where to look for an optimum solution
           [100., 250.]]
...
data = [["Power", "kW", "%8.1f", var[0]+"/1000."],
        ...
        ["Cruise_SFC", "kg/kW/h", "%8.4f", "aircraft.performance.mission.crz_tsfc"],
...
print(ac.airframe.wing.area)
print('longeur fuselage:', ac.airframe.body.length)
print(f'aircraft{ac}')
eco = Economics(ac)
eco.operating_cost_analysis()
pec = Performance(ac)
print(f'coc: {eco.cash_op_cost}')
print(f"pec.take_off.tofl_eff: {pec.take_off.tofl_req}")
